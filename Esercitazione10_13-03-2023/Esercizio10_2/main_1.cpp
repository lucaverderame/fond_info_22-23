...
double sum_inverse(double a[], int n)
{
    double s = 0.0;
    for (int i = 0; i < n; i++)
        if (a[i] != 0.0)
...
int main() 
{
    ...
    const int nf = 6;
    double v[dim];
    double (*pf[nf])(double[], int) = {minimum, maximum, sum, sum_square, mean, sum_inverse};
	...
	do {
        ...
        cout << "5 - Media" << endl;
        cout << "6 - Somma degli inversi" << endl;
		cin >> scelta;
        if (scelta >= 1 && scelta <= nf) {
            double risultato = (*pf[scelta - 1])(v, dim);
            cout << "Risultato: " << risultato << endl;
        }
		else
			cout << "Errore: scegliere un valore tra 1 e " << nf << endl;
	} while (scelta < 1 || scelta > nf);
	return 0;
}
